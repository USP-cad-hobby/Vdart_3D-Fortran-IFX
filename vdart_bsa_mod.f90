...
contains
  ...
  subroutine bsa(indi, inb, ins, ih, sw)
    ...
    do i = 1, nblades
      do k = indi1, km
        ! Bound/shed vortex contributions (spanwise segments)
        do j = 1, noli
          ! Skip spanwise segment for J=NOL1 (no J+1 exists)
          ! But DON'T skip trailing vortex computation!
          if (j /= noli) then
          ...
          ! Trailing (streamwise) vortex from K to K+1
          ! Legacy: IF(K.EQ.KMNET) GOTO 7 - skip for last K
          if (k < km) then
            do l = 1, 3
              x(l) = H1(i, j, k, l)
              y(l) = H1(i, j, k+1, l)
    ...
    ! CRITICAL FIX: Must loop over ALL blades, not just blade 1 (legacy lines 1005-1026)
    if (indi1 /= 1) then
      do i = 1, nblades
        do j = 1, noli
          do l = 1, 3
